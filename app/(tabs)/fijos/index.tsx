...
const GastosFijosScreen = () => {
	...
	const mutation = useMutation({
		mutationFn: ({
			...
			updateServiciosMensualesServices(
				id,
				nuevoEstado,
				fechaActualizacion,
			),
	...
	const { start, canStart, TourGuideZone, eventEmitter } =
		useTourGuideController('fijos');
	...
	const handleToggleEstado = (item: GastoFijo) => {
		const nuevoEstado: EstadosEnum =
			item.estado === 'Pagado'
				? EstadosEnum.Pendiente
				: EstadosEnum.Pagado;
						...
						onPress: () =>
							mutation.mutate({
								...
								fechaActualizacion: new Date()
									.toISOString()
									.split('T')[0], // Envía fecha YYYY-MM-DD
							}),
					},
				],
			);
	...
	const renderItem = ({ item }: { item: GastoFijo }) => (
		<View className='flex-row items-center justify-between bg-white p-4 mx-4 mb-3 rounded-2xl shadow-sm border border-gray-100'>
			...
			<View className='flex-1 items-center'>
				<View
					className={`w-24 items-center px-3 py-2 rounded-xl border ${item.estado === EstadosEnum.Pagado ? 'border-green-400 bg-green-100' : 'border-amber-400 bg-amber-100'}`}>
					<Text
						className={`text-xs font-bold ${item.estado === EstadosEnum.Pagado ? 'text-green-700' : 'text-amber-700'}`}>
						{item.estado}
					</Text>
				</View>
			</View>
			...
			<View className='flex-1 items-end '>
				<Pressable
					...
					{mutation.isPending ? (
						...
						<Text
							className={`text-xs font-bold ${item.estado === EstadosEnum.Pagado ? 'text-amber-700' : 'text-white'}`}>
							{item.estado === EstadosEnum.Pagado
								? 'Listo'
								: 'Pagar'}
						</Text>
					)}
	...
	const pendientes = gastos.filter(
		(g) => g.estado === EstadosEnum.Pendiente,
	).length;
	const pagados = gastos.filter(
		(g) => g.estado === EstadosEnum.Pagado,
	).length;
			...
			<TourGuideZone
				...
				<View className='flex-row justify-between px-4 mt-2 mb-1'>
					<View className='flex-1 bg-amber-100/60 border border-amber-300 p-4 rounded-2xl mx-1 shadow-sm items-center'>
						...
						<Text className='text-2xl font-bold text-amber-800 mt-1'>
							{pendientes}
						</Text>
					</View>
